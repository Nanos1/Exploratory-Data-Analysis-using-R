...
data %>%
  ...
  labs(x = "Date",
       ...
       title = "People Fully & Partially Vaccinated Ratio in Greece")

gr_popul = mean(data[ grep("Greece", data$country_region) , ]$population)

likegr_eu = data[data$population >= gr_popul*0.9 & data$population <= gr_popul*1.1 & data$continent_name =="Europe",]
...
likegr_eu %>%
  ...
  labs(x = "Date",
       ...
       title = "People Fully Vaccinated Ratio in Europe's countries that have approximately the same population with Greece")+ theme(plot.title = element_text(size=10))
...
ggplot(sa, aes(x=country_region, y=(people_fully_vaccinated), fill=country_region)) +
  ...
  scale_fill_brewer(palette="Paired") +
  labs(x = "South America Countries", y = "People Fully Vaccinated", title = "People Fully Vaccinated in South America") + theme(plot.title = element_text(size=10)) + theme_minimal() +
  scale_x_discrete(guide = guide_axis(angle = 90))

ggplot(sa, aes(x=country_region, y=(fully_vaccinated_ratio), fill=country_region)) +
  geom_bar(stat="identity", color="black", position=position_dodge(1), size=.5) +
  scale_fill_brewer(palette="Paired") +
  labs(x = "South America Countries", y = "Fully vaccinated ratio", title = "People Fully Vaccinated Ratio in South America") + theme(plot.title = element_text(size=10)) + theme_minimal() +
  scale_x_discrete(guide = guide_axis(angle = 90))
